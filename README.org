...
The fifth is open and is a packaging decision rather than a bug to patch:
*=nix build= now fails, and should.* River bumped =river_window_manager_v1= from
4 to 5 in a patch release, the flake pins a channel rather than a river, and the
vendored protocol is version 4 — so the flake was building a session entry that
could not start, and succeeded, because nothing in that build had ever connected
to the river it pins. =nix-shell=, =make=, =make check= and CI are unaffected
and green. [[file:PLAN.org::#log8][PLAN.org §log8]] has the two ways out.
