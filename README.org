...
shifted glyph has to be declared rather than derived. If you are not on a US
keyboard, this is the one line to set before the prompt is typeable:
...
render sequence, the shared-memory overlay path, and a real client window being
announced, placed and sized.
