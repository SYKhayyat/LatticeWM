...
*Lockers are two different things now, and it is worth being exact.* A layer
surface can take the keyboard *exclusively*, and LatticeWM handles that: it
stops sending focus requests, and stops drawing a focused border on a window
that does not have focus. Without it a window manager re-focuses a window
underneath the lock screen on every manage sequence, forever.

But current lockers do not do that any more. swaylock and gtklock use
=ext-session-lock-v1=, a separate protocol the *compositor* implements, and
under it river handles the lock surface and its input itself. LatticeWM is told
— river sends =session_locked= and it is recorded on the world — and it has
nothing it needs to do. Verified: with swaylock holding an
=ext-session-lock-v1= lock over a nested river, the window manager stays alive,
the tree keeps its windows, and commands still run. Killing the locker does
*not* unlock the session, which is the protocol working as designed rather than
a bug.

So the layer-shell branch is for the lockers that predate that protocol, and
the exclusive-focus path is exercised by nothing here — an honest gap, and a
narrow one.
...
- *libinput hardware* — tap-to-click, acceleration, click and scroll method are
  ...
  behind them. This is the one that needs a machine rather than a better
  harness;
- *a pointer press* — the two pointer bindings river makes are counted live and
  ...
  input gets close and not there: =wlrctl= offers =click= and no separate
  press/release, so the displacement half of a drag cannot be produced at all,
  and a Super+click through a virtual pointer produced nothing observable —
  which does not distinguish "the binding did not fire" from "the synthetic
  press never reached the matcher", so it is reported as untested rather than as
  a defect;
- *a layer-shell locker taking exclusive keyboard focus* — the branch exists and
